"""Sweep evals across multiple GPT-5 family models and produce a comparison table.

Calls 5-experiments/run.py (which targets 4-two-step-app) for each model,
then aggregates per-model metrics into a comparison JSON.

Usage:
    python 5-experiments/sweep.py
    python 5-experiments/sweep.py --models gpt-5,gpt-5-mini,gpt-5.4
    python 5-experiments/sweep.py --reasoning-effort low
"""
...
def run_eval(model: str, reasoning_effort: str | None = None) -> dict | None:
    """Run eval for a single model via 5-experiments/run.py, return parsed results."""
    safe = model.replace(".", "_")
    out_file = RESULTS_DIR / f"twostep_{safe}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Sweep two-step app across models")
    parser.add_argument("--models", default=",".join(DEFAULT_MODELS),
                        help="Comma-separated list of model deployment names")
    ...
    all_results: dict = {}
    for model in models:
        data = run_eval(model, reasoning_effort=args.reasoning_effort)
        if data:
